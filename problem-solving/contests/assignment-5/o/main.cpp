...
{
  if (nr > ROWS || nc > COLS || visited[nr][nc] == vid || grid[cr][cc] < grid[nr][nc])
    return;
  visited[nr][nc] = visited[cr][cc] = vid;
  ui old_current_elem = grid[cr][cc];
  ui old_next_elem = grid[nr][nc];
  grid[nr][nc] = grid[cr][cc] = table[grid[cr][cc]][grid[nr][nc]];
  solve(cr, cc + 1);
  grid[cr][cc] = old_current_elem;
  grid[nr][nc] = old_next_elem;
  visited[nr][nc] = visited[cr][cc] = 0;
}
